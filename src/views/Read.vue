  ...
  <div class="blog-read-wrap">
    <div>
      <div class="p-a-xxxxsm m-l-auto m-r-auto container m-b-xxxxsm flex sm-flex sm-column">
        <router-link to="/"><h1 class="txt-c-white m-t-auto m-b-auto">Adewale's Blog</h1></router-link><div class="flex txt-s-sm  space-between full-width m-t-auto m-b-auto m-l-auto sm-m-l-none sm-m-r-auto sm-m-t-xxxsm" style="max-width: 200px">
        <a href>
      ...
      <div class="full-width bg-c-white bd-round-a-xxxxsm container sm-p-a-xxxsm">
        ...
        <h1>{{title}}</h1><span class="m-t-auto m-b-auto txt-c-grey-shade-2 txt-s-xxsm">By: {{author}}</span><span class="txt-bold m-l-xxsm">&middot;</span><span class="m-l-xxsm txt-s-xxsm txt-c-grey-shade-2">{{time}}</span>
        </div>
...
export default {
    ...
    async created(){
        try{
            let blogs = await axios.get(`/api/1/blog/fetch/${this.$route.params.post_id}`);
